...
export default function GameOver() {
  ...
  return (
    <div className="fixed z-10 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
      {/* Background Overlay */}
      <div className="fixed inset-0 bg-nymBackground bg-opacity-75 transition-opacity" aria-hidden="true"></div>
      {/* Modal Content */}
      <div className="flex items-center justify-center min-h-screen">
      <Card className="w-full max-w-md z-20 mx-6 bg-nymText">
            <CardHeader>
                <CardTitle className='font-montserrat font-black'>Game Over</CardTitle>
            </CardHeader>
            <CardContent>
                <div className='flex flex-col gap-2'>
                    ...
                    <div className='flex flex-col gap-2'>
                        <p className="font-cabin ">Words Matched: {matchedSynonyms.length} match{matchedSynonyms.length > 1 ? "es" : ""}.</p>
                        <div
                        className='flex gap-1 flex-wrap justify-start'
                        >
                            {matchedSynonyms.map((synonym, index) => (
                                <p key={index} className="font-cabin text-sm italic bg-nymBackground w-fit p-1 px-2 rounded-sm text-nymText">{synonym}</p>
                            ))}
                        </div>
                    ...
                    <div>
                    <p className="font-cabin text-lg font-bold">Total Score: {totalScore} points</p>
                    </div>
                </div>
            </CardContent>
            <CardFooter>
                ...
                className="transition-all ease-in-out w-full bg-nymBackground text-nymText p-2 rounded-sm font-black tracking-wider hover:invert hover:border-nymBackground">Home</button>
            </CardFooter>
        </Card>
      </div>
    </div>
